---
source_file: "raw/ocr/CONCEPTS_OF_POLLUTION_IN_LATE_SASANIAN_I.md"
type: "document"
community: "Iranian Kingship"
location: "chunk 3/3"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Iranian_Kingship
---

# Pišksilite School / Pišksil

## Connections
- [[Abargite School  Abarg]] - `contrasts_with` [EXTRACTED]
- [[CONCEPTS OF POLLUTION IN LATE SASANIAN IRAN DOES POLLUTION NEED STAIRS, AND DOES IT FILL SPACE]] - `references` [EXTRACTED]
- [[Claim tuhīgīh as Empty-Space Pollution Occupying Consolidated Three-Dimensional Space]] - `conceptually_related_to` [EXTRACTED]
- [[Daštānestān Menstrual Hut Pollution Case (TD2 p. 573)]] - `participates_in` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Iranian_Kingship